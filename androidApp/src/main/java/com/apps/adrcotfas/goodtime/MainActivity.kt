...
class MainActivity : ComponentActivity(), KoinComponent {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContent {
            val themeSettings by settingsRepository.settings.map {
                Pair(it.uiSettings.useDynamicColor, it.uiSettings.darkModePreference)
            }.collectAsStateWithLifecycle(
                initialValue = Pair(false, DarkModePreference.SYSTEM)
            )
            val dynamicColor = themeSettings.first
            val darkTheme = themeSettings.second == DarkModePreference.DARK ||
                    (themeSettings.second == DarkModePreference.SYSTEM && isSystemInDarkTheme())
